(function(global, factory) {
    ...
    function isDom(value) {
        return (
            typeof value === "object" && typeof value.nodeType === "number" && typeof value.ownerDocument === "object"
        );
    }
            ...
            if (otherStyle) {
                for (var name in otherStyle) {
                    if (
                        ...
                        (overwrite === true ||
                            (overwrite === false ? !this.hasOwnProperty(name) : otherStyle[name] != null))
                    ) {
    ...
    function normalizeStyle(style) {
        if (style) {
            ...
            style.textVerticalAlign =
                textVerticalAlign == null || VALID_TEXT_VERTICAL_ALIGN[textVerticalAlign] ? textVerticalAlign : "top";
    ...
    function needDrawText(text, style) {
        return (
            text != null &&
            (text || style.textBackgroundColor || (style.textBorderWidth && style.textBorderColor) || style.textPadding)
        );
    }
    ...
    var Painter = function(root, storage, opts) {
        ...
        var singleCanvas = !root.nodeName || root.nodeName.toUpperCase() === "CANVAS"; // In node ?
                ...
                if (item.added) {
                    ...
                    for (var k = 0; k < item.count; k++) {
                        ...
                        prevSvgElement = svgElement =
                            getTextSvgElement(displayable) || getSvgElement(displayable) || prevSvgElement;
    ...
    if (!env$1.canvasSupported) {
            ...
            for (i = 0; i < dataLength; ) {
                ...
                switch (cmd) {
                    ...
                    case A:
                        ...
                        str.push(
                            type,
                            round(((cx - rx) * sx + x) * Z - Z2),
                            comma,
                            round(((cy - ry) * sy + y) * Z - Z2),
                            comma,
                            round(((cx + rx) * sx + x) * Z - Z2),
                            comma,
                            round(((cy + ry) * sy + y) * Z - Z2),
        ...
        ZImage.prototype.brushVML = function(vmlRoot) {
            ...
            if (hasRotation) {
                ...
                vmlElStyle.filter =
                    imageTransformPrefix + ".Matrix(" + transformFilter.join("") + ", SizingMethod=clip)";
            } else {
                if (m) {
                    x = x * scaleX + m[4];
                    y = y * scaleY + m[5];
